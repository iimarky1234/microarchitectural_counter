...
$PERF_PREFIX perf stat -x, -r "$RUNS" -e "$EMR_EVENTS" taskset -c "$CPU_CORE" $TARGET_CMD >/dev/null 2>"$PERF_RAW_CSV" || {
  echo -e "${YELLOW}Notice: Retrying with standard hardware event aliases...${NC}"
  $PERF_PREFIX perf stat -x, -r "$RUNS" -e "cycles,instructions,branches,branch-misses,L1-dcache-load-misses,L1-dcache-loads,duration_time" taskset -c "$CPU_CORE" $TARGET_CMD >/dev/null 2>"$PERF_RAW_CSV" || true
}
